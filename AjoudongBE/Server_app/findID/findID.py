        ...
        try:
            ...
            if UserAccount.objects.filter(uSchoolID = uSchoolID).exists():
                ...
                if(len(uID) < 5):
                    ...
                    return JsonResponse({'response' : 1, 'message' : maskedID}, status = 200)
                elif(len(uID) < 3):
                    return JsonResponse({'response' : 2, 'message' : "Too Short ID"}, status = 200)
                ...
                return JsonResponse({'response' : -1, 'message' : "No matching ID"}, status = 200)
...
class getEntireID(View):
    ...
    def post(self, request):
        ...
        try :
            print(data)
            user = UserAccount.objects.get(uSchoolID = data['uSchoolID'])
            body = {
	            "templateSid" : 1606,
	            "recipients" : 
	                [
